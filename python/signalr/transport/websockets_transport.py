...
class WebSocketsTransport(BaseTransport):
    ...
    async def _handshake(self):
        ...
        try:
            ...
            await self._connection.send(
                json_dumps(handshake_request_data)
            )
            await self._connection.send(MESSAGE_TERMINATOR_SYMBOL)
    ...
    async def recv(self) -> Optional[Dict]:
        try:
            ...
            raise WebSocketTransportError(f'Message decode error on recv: {error}')
